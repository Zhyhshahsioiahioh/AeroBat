...
def rotate_vector(v, angle, p = None):   #利用罗德里格斯公式，在三维空间中旋转向量
    if p is None:           
        if v[0] != 0 or v[1] != 0:
            p = np.array([v[1], -v[0], 0])
        else:
            p = np.array([1, 0, 0]) if v[2] != 0 else np.array([0, 1, 0])
   
    p = p / np.linalg.norm(p)

    v_rot = v * np.cos(angle) + np.cross(p, v) * np.sin(angle) + p * np.dot(p, v) * (1 - np.cos(angle))
...
def get_energy_cost(attacker, defender, target):
    ...
    while t<3:
        ...
        if np.linalg.norm(target_.state.p_vel)<0.001 or np.linalg.norm(defender_.state.p_vel)<0.001:
            break
        #attacker_act = attacker_.action_callback(target_, attacker_, defender_)
        attacker_act = attacker.action.u
        #denefder_act = defender_.action_callback(target_, attacker_, defender_)
        denefder_act = defender_.action_callback(attacker_, defender_)
